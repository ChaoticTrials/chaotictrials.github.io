...
<ProjectHeader slug="curse-of-curses" mcVersion="1.20" />
...
## Cooldown Setting
`cooldownSetting` defines the type of nights in which curses are applied. Possible values are:

- `EVERY_NIGHT`: Curses are applied every night.
- `FULL_MOON`: Curses are only applied during a full moon.
- `NEW_MOON`: Curses are only applied during a new moon.

## Denylisted Curses
If there are certain curses you do not want applied by this mod, you can use the `denylistCurses` option in the
