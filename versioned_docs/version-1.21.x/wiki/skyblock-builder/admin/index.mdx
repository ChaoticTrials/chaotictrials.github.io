...
## Managing Teams
### Create
To create a team with a specific name, use the command: `/skyblock manage teams create <name>`. This will create a team
...
There's also a command `/skyblock manage teams createAndJoin <name>`. This may be used to create a team and instantly
...
`/skyblock manage teams clear`

Refer to [Deleting Teams](#delete) for more information.

If you use `/skyblock manage teams clear <name>` with a name provided, all players in that team will be removed and
teleported to spawn island.

## Modify Existing Teams
### Add Players
You may add players to a team using `/skyblock manage addPlayer <players> <name>` to add one or more players to a team
...
team members, who knows. You that, you may use `/skyblock manage kickPlayer <players>`.
...
new team, you may want to use `/skyblock manage islandShape <name>`. This will change the current template to the other
