...
<ProjectHeader slug="automatic-tool-swap" mcVersion="1.20" />
...
- **Description:** Choose the mode for how to detect items:
  - `TAG`: Default mode, works best with modpacks containing other tool mods.
  - `VANILLA`: Use this if `TAG` mode doesn't work, especially on vanilla servers.

Allowed Values: `TAG`, `VANILLA`
