...
# Items
## Backpack
The Backpack is a useful item for increasing your storage capacity. By default, it provides 9 slots for carrying items. You can expand its capacity up to 54 slots by combining it with other backpacks. This limit is configurable.

When merging backpacks, their colors will blend together, and items stored in them will remain safe. You can also customize the Backpack’s color using dyes.

To open the Backpack, use the default key `O`. Additionally, you can equip it in the Curios slot (`curios:back`).

:::warning
At the moment, the Backpack is incompatible with sorting mods and can lead to losing items. See this issue on
[GitHub](https://github.com/ChaoticTrials/UtilitiX/issues/66). If you still read this warning when the issue is closed,
please reach out to me to remove this warning.
:::
...
## Ancient Compass
The Ancient Compass points to the nearest Ancient City. It can be found in spawner room chests with a 3.125% chance.
